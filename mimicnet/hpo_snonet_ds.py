...
def sample_config(trial: optuna.Trial):
    ...
    model_params = {
        ...
        'init_depth': trial.suggest_int('init_depth', 1, 5),
        'bias': True,
        'max_odeint_days': trial.suggest_int('max_odeint_days', 8 * 7, 16 * 7,
                                             7)
    }
...
def create_model(config, patient_interface, train_ids):
    ...
    return SNONETDiagStat(subject_interface=patient_interface,
                          diag_gram=diag_gram,
                          **config['model'],

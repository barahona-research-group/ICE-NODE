...
import os
from lib import AbstractConfig
from lib.utils import write_config, load_config
from lib.base import configuration_class_registry
...
class ConfigTest(unittest.TestCase):
    ...
    def test_registration(self):
        ...
        for conf in self.all_configs:
            ...
            self.assertIn(name, configuration_class_registry)
            self.assertEqual(configuration_class_registry[name], type(conf))
    ...
    def test_to_dict(self):
        ...
        for conf in self.all_configs:
            ...
            leaves = jtu.tree_leaves(
                eqx.filter(conf_dict,
                           lambda x: issubclass(type(x), AbstractConfig)))
            self.assertCountEqual(leaves, [])

    def test_from_dict(self):
        for conf in self.all_configs:
            conf_dict = conf.to_dict()
            conf_ = AbstractConfig.from_dict(conf_dict)

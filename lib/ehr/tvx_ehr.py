...
class TVxEHR(AbstractDatasetRepresentation):
    ...
    def save(self, path: Union[str, Path], overwrite: bool = False,
             save_dataset: Optional[Union[str, Path, bool]] = True):
        """Save the Patients object to disk.

        Args:
            path (Union[str, Path]): path to save the Patients object.
            overwrite (bool, optional): whether to overwrite existing files. Defaults to False.
            save_dataset (Optional[Union[str, Path, bool]], optional): path to save the dataset. Defaults to True.
                if None or False, do not save the dataset. If True, the dataset representation
                will be stored in a path generated by the function dataset_path_prefix. If a string or Path,
                the dataset representation will be saved in the given path.
        """
        ...
        if save_dataset is None or save_dataset is False:
            return

        dataset_path = self.dataset_path_prefix(path) if save_dataset == True else save_dataset
        self.dataset.save(dataset_path, overwrite)

    @classmethod
    def load(cls, path: Union[str, Path],
             dataset: Optional[Union[str, Path, Dataset]] = None) -> TVxEHR:
        """Load the Patients object from disk.

        Args:
            path (Union[str, Path]): path to load the Patients object from.
            dataset (Optional[Union[str, Path, Dataset]], optional): dataset object or path to load the dataset from.
            Defaults to None. if None, the dataset representation will be loaded from a path generated by
            the function dataset_path_prefix.
        Returns:
        ...
        if isinstance(dataset, Dataset):
            ...
            dataset = Dataset.load(cls.dataset_path_prefix(path))
        else:
